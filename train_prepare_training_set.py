...
def prepare_training_set(
    camera, target_pipeline, dev_settings, n_images=150, root_dir="./data/"
):
    ...
    log.info(
        "Collected {} landscape-oriented photos for training".format(
    ...
    for nef_file in utils.progress_bar(
        raw_filenames_selected, f"Preparing train. data ({camera})"
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare training pairs (raw inputs in *.npy and optimization targets in *.png) for a given camera"
    )
