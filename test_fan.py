...
def restore_flow(
    filename, isp, manipulations, jpeg_qf, jpeg_codec, dcn_model, patch_size
):
    ...
    try:
        accuracy = helpers.utils.get(
            training_log, "forensics.performance.accuracy.validation"
        )[-1]
    except:
        accuracy = np.nan
    ...
    flow = manipulation_classification.ManipulationClassification(
        ...
        manipulations,
        distribution,
        coreutils.getkey(training_log, "forensics/args"),
        {},
...
def main():
    parser = argparse.ArgumentParser(
        description="Test manipulation detection (FAN) on RGB images"
    )
    ...
    group.add_argument(
        ...
        action="store",
        default=None,
        help="Included manipulations, e.g., : {}".format(
            "sharpen,jpeg,resample,gaussian"
        ),
    )
    ...
    if args.isp == "ONet":
        data = dataset.Dataset(
            ...
            load="y",
            val_rgb_patch_size=2 * args.patch,
            val_n_patches=args.patches,
        )
    else:
        data = dataset.Dataset(
            args.data,
    ...
    for filename in json_files:
        ...
        if args.re is None or re.findall(args.re, filename):
            ...
            print(
                "Accuracy validated/expected: {:.4f} / {:.4f}".format(
                    np.mean(np.diag(conf)), accuracy
                )
            )
            print(
                results_data.confusion_to_text(
                    (100 * conf).round(0), flow._forensics_classes, filename, "txt"
                )
            )
        else:
