...
def validate(
    model, data, out_directory, savefig=False, epoch=0, show_ref=False, loss_metric="L2"
):
    ...
    if savefig:
        ...
        fig = Figure(
            figsize=(40, 1.1 * 40 / images_x * images_y * (1 if not show_ref else 0.5))
        )
    ...
    if savefig:
        ...
        fig.savefig(
            os.path.join(out_directory, "validation_{:05d}.jpg".format(epoch)),
            bbox_inches="tight",
            dpi=150,
        )
        del fig
...
def show_progress(isp, out_directory):
    ...
    fig.savefig(
        os.path.join(out_directory, "progress.png"), bbox_inches="tight", dpi=150
    )
    del fig
...
def train_nip_model(
    ...
    n_epochs=10000,
    lr_schedule=None,
    validation_loss_threshold=1e-3,
    validation_schedule=100,
    resume=False,
    patch_size=64,
    batch_size=20,
    data=None,
    out_directory_root="./data/models/nip",
    save_best=False,
    discard="flat",
):
    ...
    try:
        ...
        if batch_x.shape != (5, patch_size, patch_size, 4) or batch_y.shape != (
            5,
            2 * patch_size,
            2 * patch_size,
            3,
            ...
            raise ValueError(
                "The training batch returned by the dataset instance is of invalid size!"
            )
    ...
    if batch_size > data.count_training or batch_size > data.count_validation:
        raise ValueError(
            f"Batch size ({batch_size}) exceeds dataset size ({data.count_training}/{data.count_validation})!"
        )
        ...
        for epoch in range(start_epoch, n_epochs):
            ...
            for batch_id in range(n_batches):
                batch_x, batch_y = data.next_training_batch(
                    batch_id, batch_size, patch_size, discard=discard
                )
            ...
            if epoch % validation_schedule == 0:
                ...
                ssims, psnrs, v_losses, developed = validate(
                    model,
                    data,
                    out_directory,
                    True,
                ...
                if not save_best or (
                    len(model.performance["loss"]["validation"]) > 2
                    and model.performance["loss"]["validation"][-1]
                    <= min(model.performance["loss"]["validation"])
                ):
                ...
                if len(model.performance["loss"]["validation"]) > 5:
                    if model.performance["loss"]["validation"][-1] > 1.2 * min(
                ...
                if (
                    validation_loss_threshold is not None
                    and len(model.performance["loss"]["validation"]) > 10
                ):
                    current = np.mean(
                        model.performance["loss"]["validation"][-n_tail:-1]
                    )
                    previous = np.mean(
                        model.performance["loss"]["validation"][-(n_tail + 1) : -2]
                    )
                    vloss_change = abs((current - previous) / previous)

                    if vloss_change < validation_loss_threshold:
                        print(
                            "Early stopping - the model converged, validation loss change {}".format(
                                vloss_change
                            )
                        )
                        break
    ...
    training_summary["Epoch"] = epoch
    if not save_best or (
        model.performance["loss"]["validation"][-1]
        <= min(model.performance["loss"]["validation"])
    ):
        training_summary["Saved checkpoint"] = epoch
...
def train_nip_bare(
    ...
    out_directory = os.path.join(
        out_directory_root, camera_name, model.model_code, model.scoped_name
    )
    ...
    with tqdm(
        total=n_epochs,
        ncols=TQDM_WIDTH,
        desc="{} for {}".format(model.model_code, camera_name),
    ) as pbar:
        pbar.update(start_epoch)
