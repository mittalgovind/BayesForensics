...
def develop_image(
    ...
    if isinstance(model, pipelines.ClassicISP):
        print(
            "Configuring ISP to CFA: {} & sRGB {}".format(cfa, srgb.round(2).tolist())
        )
    ...
    plots.image(
        ...
        "{}, PSNR={:.1f} dB, SSIM={:.2f} : {{}}".format(
            model.model_code, float(psnrs.mean()), float(ssims.mean())
        ),
        axes=axes[0],
    )
...
def main():
    ...
    parser.add_argument(
        ...
        dest="nip",
        action="store",
        help="model name / path to a trained ISP model",
    )
    parser.add_argument(
        "-i", "--image", dest="image", action="store", help="path to a RAW image"
    )
