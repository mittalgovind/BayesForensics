...
def get_parameters(csv_file, metrics=("ssim", "psnr", "loss", "params")):
    ...
    parameters = pd.DataFrame(
        columns=["scenario", "label", "active", "run_group", "params", "model_code"]
    )

    if csv_file is not None:
        parameters = parameters.append(
            pd.read_csv(csv_file), ignore_index=True, sort=True
        )
...
def main():
    ...
    parser.add_argument(
        ...
        dest="data_dir",
        action="store",
        default="./data/raw/training_data/",
    ...
    parser.add_argument(
        "--ha",
        dest="hyperparams_args",
        default=None,
        help="Set hyper-parameters / override CSV settings if needed (JSON string)",
    )
    parser.add_argument(
        "--hp",
        dest="hyperparams_csv",
        default=None,
        help="CSV file with hyper-parameter configurations",
    )
    parser.add_argument(
        "--resume",
        dest="resume",
    ...
    if not hasattr(pipelines, args.nip) or not issubclass(
        getattr(pipelines, args.nip), pipelines.NIPModel
    ):
        raise ValueError(
            "Invalid NIP model ({})! Available NIPs: ({})".format(
                args.nip, pipelines.supported_models
            )
        )
    ...
    threshold_label = (
        f"(convergence threshold {utils.format_number(convergence_threshold)})"
    ...
    if not args.dry:
        ...
        for key in ["Training", "Validation"]:
            print(
                "{:>16s} [{:5.1f} GB] : X -> {}, Y -> {} ".format(
                    "{} data".format(key),
                    helpers.debugging.mem(data[key.lower()]["x"])
    ...
    for counter, (index, params) in enumerate(
        parameters.drop(
            columns=["scenario", "label", "params", "model_code"]
        ).iterrows()
    ):
        ...
        if not args.dry:
            print(
                "## {} : Scenario #{} - {} / {}".format(
                    args.nip, index, counter + 1, len(parameters)
                )
            )
        ...
        # Run training
        if not args.dry:
            out_dir = train_nip_model(
                model,
                args.camera,
                args.epochs,
                validation_loss_threshold=convergence_threshold,
        ...
        if args.fill is not None:
            ...
            if len(model.performance["loss"]["validation"]) > 0:
                for key in ["ssim", "psnr", "loss"]:
                    parameters.loc[index, key] = model.pop_metric(
                        key, "validation"
                    )  # results['performance'][key]['validation'][-1]
                ...
                if os.path.isfile(results_json):
                    ...
                    for key in ["ssim", "psnr", "loss"]:
                        parameters.loc[index, key] = utils.get(
                            results, f"performance.{key}.validation"
                        )[-1]
    ...
    if args.fill is not None:
        ...
        if args.fill == "-":
            ...
            raise ValueError(
                "Invalid value for the output results file: {}".format(args.fill)
            )
    ...
    if args.dry:
        ...
        for index, key in enumerate(sorted(model_log.keys())):
            print(
                "{}  {:3d}. {} -> {}".format(
                    " " if len(model_log[key]) == 1 else "!", index, key, model_log[key]
                )
            )
