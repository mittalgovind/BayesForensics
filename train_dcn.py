...
def main():
    ...
    try:
        ...
        if args.dcn_param_list is not None:
            parameters = parameters.append(
                pd.read_csv(args.dcn_param_list), ignore_index=True, sort=True
            )
    ...
    args.epochs = (
        int(np.ceil(args.epochs / args.validation_schedule) * args.validation_schedule)
        + 1
    )
    ...
    for counter, (index, params) in enumerate(
        ...
        dcn = getattr(compression, args.dcn)(
            patch_size=training_spec["patch_size"], **dcn_params
        )
        ...
        if args.fill is not None:
            ...
            if os.path.isfile(results_json):
                ...
                parameters.loc[index, "ssim"] = results["performance"]["ssim"][
                    "validation"
                ][-1]
                parameters.loc[index, "loss"] = results["performance"]["loss"][
                    "validation"
                ][-1]
                parameters.loc[index, "entropy"] = results["performance"]["entropy"][
                    "training"
                ][-1]
