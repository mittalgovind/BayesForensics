...
class JPEGDoubleCompression(BayesBaseModel, ABC):
    ...
    def _create_model(self):
        ...
        layers.append(tf.keras.layers.Conv2D(filters//self.filter_multiplier, 1, padding='same', activation=self.activation)
...
class Ensemble(tf.keras.Model):
    ...
    def call(self, inputs, training=False, full_output=True):
        outputs = self._model(inputs, training=training)
        if full_output:
            return outputs
    ...
    def test_step(self, data):
        x, y = data
        y_pred = self.call(x, training=False, full_output=False)
