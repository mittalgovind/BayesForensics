...
def graph_uncertainties_by_method(results_dir, figs_dir):
    ...
    for i in range(len(methods)):
        ...
        sns.boxplot(
            data=list(uncertainties["var_ratio"].values()),
            ax=axes[3 * i + 0],
            showfliers=False,
            color="steelblue",
        )
        ...
        sns.boxplot(
            data=list(uncertainties["pred_ent"].values()),
            ax=axes[3 * i + 1],
            showfliers=False,
            color="steelblue",
        )
        ...
        sns.boxplot(
            data=list(np.abs(list(uncertainties["mut_info"].values()))),
            ax=axes[3 * i + 2],
            showfliers=False,
            color="steelblue",
        )
...
def get_different_method_from_indices(experiment_list, indices, method, num_samples):
    ...
    for ind in indices:
        while not found:
            ...
            if elm["method"] == method and np.isclose(
                elm["sf"], experiment_list[ind]["sf"]
            ):
                found = True
                break
...
def graph_quantiles(experiments_source, image_output, training_method, num_samples):
    ...
    full_indices = [
        indices,
        get_different_method_from_indices(
            exp_list, indices, other_methods[0], num_samples
        ),
        get_different_method_from_indices(
            exp_list, indices, other_methods[1], num_samples
        ),
        get_different_method_from_indices(
            exp_list, indices, other_methods[2], num_samples
        ),
    ]
    ...
    for i in range(len(methods)):
        sns.kdeplot(data=distribs["var_ratio"][methods[i]], ax=axes[-12 + i], fill=True)
        axes[-12 + i].set_xlim(left=0)

        sns.kdeplot(data=distribs["pred_ent"][methods[i]], ax=axes[-8 + i], fill=True)
        axes[-8 + i].set_xlim(left=0)

        sns.kdeplot(data=distribs["mut_info"][methods[i]], ax=axes[-4 + i], fill=True)
...
def graph_all_quantiles(results_dir, figs_dir, num_samples):
    ...
    for method in methods:
        graph_quantiles(
            os.path.join(results_dir, f"{method}.pkl"),
...
def graph_uncertainties_by_sf(results_dir, figs_dir):
    ...
    for method in methods:
        ...
        uncertainties = [
            mean_uncertainty_by_sf(exp_list, "var_ratio"),
...
def get_different_quality_from_indices(experiment_list, indices, quality, num_samples):
    ...
    for ind in indices:
        while not found:
            ...
            if elm["quality"] == quality and np.isclose(
                elm["sf"], experiment_list[ind]["sf"]
            ):
                found = True
                break
...
def graph_quantiles_jpeg(results_dir, figs_dir, method, num_samples):
    ...
    full_indices = [
        indices,
        get_different_quality_from_indices(
            exp_list, indices, qualities[1], num_samples
        ),
        get_different_quality_from_indices(
            exp_list, indices, qualities[2], num_samples
        ),
        get_different_quality_from_indices(
            exp_list, indices, qualities[3], num_samples
        ),
    ]
    ...
    for i in range(len(qualities)):
        sns.kdeplot(
            data=distribs["var_ratio"][qualities[i]], ax=axes[-12 + i], fill=True
        )
        axes[-12 + i].set_xlim(left=0)

        sns.kdeplot(data=distribs["pred_ent"][qualities[i]], ax=axes[-8 + i], fill=True)
        axes[-8 + i].set_xlim(left=0)

        sns.kdeplot(data=distribs["mut_info"][qualities[i]], ax=axes[-4 + i], fill=True)
        axes[-4 + i].set_xlim(left=0)
...
def main():
    ...
    if args.figs == "quant":
        ...
        plot_acc_graphs(results_dir=args.results_dir, figs_dir=args.figs_dir)

    elif args.figs == "unc_sf":
        graph_uncertainties_by_sf(results_dir=args.results_dir, figs_dir=args.figs_dir)

    elif args.figs == "unc_method":
        graph_uncertainties_by_method(
            results_dir=args.results_dir, figs_dir=args.figs_dir
        )

    elif args.figs == "quant_jpeg":
        graph_quantiles_jpeg(
            results_dir=args.results_dir,
            figs_dir=args.figs_dir,
