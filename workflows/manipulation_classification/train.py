...
from workflows.manipulation_classification import validate
...
def train_manipulation_nip(flow, training, data, directories=None, overwrite=False):
    ...
    if any([x not in training for x in required_keys]):
        raise RuntimeError(
            "Missing keys in the training dictionary! {}".format(
                required_keys.difference(training.keys())
            )
        )
    ...
    try:
        if data.is_raw_and_rgb():
            batch_x, batch_y = data.next_training_batch(
                0, 1, training["patch_size"] * 2
            )
            if batch_x.shape != (
                1,
                training["patch_size"],
                ...
                1,
                2 * training["patch_size"],
                2 * training["patch_size"],
                3,
            ):
                raise ValueError(
                    f"The RAW+RGB training batch is of invalid size! {batch_x.shape}"
                )
        else:
            ...
            if batch_x.shape != (
                ...
                raise ValueError(
                    f"The RGB training batch is of invalid size! {batch_x.shape}"
                )
    ...
    logger.info(
        "Training manipulation classification: cam={} / lr={:.4f} / run={:3d} / epochs={}, root={}".format(
            training["camera_name"],
            training["lambda_nip"],
            training["run_number"],
            training["n_epochs"],
            directories["root"],
    ...
    if flow.is_trainable("nip") and flow.nip.count_parameters() == 0:
        raise ValueError(
            "It looks like you`re trying to optimize a NIP with no trainable parameters!"
        )
    ...
    if training["use_pretrained_nip"] and flow.nip.count_parameters() > 0:
        nip_dirname = os.path.join(
            directories["nip_snapshots"], training["camera_name"], flow.nip.model_code
        )
    ...
    training_summary["Channel Compression"] = (
        flow.codec.summary() if flow.codec is not None else "n/a"
    )
    training_summary["Channel Compression Parameters"] = str(
        flow._distribution["compression_params"]
    )
    ...
    training_summary["Learning rate decay schedule"] = utils.format_number(
        learning_rate_decay_schedule
    )
    training_summary["Learning rate decay rate"] = utils.format_number(
    ...
    with utils.progress_bar(
        training["n_epochs"], "Train manip. classification"
    ) as pbar:
        ...
        for epoch in range(0, training["n_epochs"]):
            ...
            for batch_id in range(n_batches):
                ...
                if data._loaded_data == "xy":
                    batch_x, batch_y = data.next_training_batch(
                        batch_id, training["batch_size"], 2 * training["patch_size"]
                    )
                else:
                ...
                comb_loss, comp_loss = flow.training_step(
                    batch_x,
                    batch_y,
                    training["lambda_nip"],
                    training["lambda_dcn"],
            ...
            for model_name, model in zip(model_list, [flow.nip, flow.fan]):
                ...
                loss_last_k_epochs[model_name].append(
                    model.pop_metric("loss", "training")
                )
            ...
            if epoch % training["validation_schedule"] == 0:
                ...
                accuracy, conf = validate.validate_fan(flow, data)
                ...
                if flow.is_trainable("nip"):
                    ...
                    values = validate.validate_nip(
                        flow.nip,
                        data,
                        nip_save_dir,
                        epoch=epoch,
                        show_ref=True,
                        loss_type=flow.nip.loss_metric,
                    )
                ...
                if flow.is_trainable("dcn"):
                    if isinstance(flow.codec, compression.DCN):
                        values = validate.validate_dcn(
                            flow.codec, data, nip_save_dir, epoch=epoch, show_ref=True
                        )
                    elif isinstance(flow.codec, jpeg.JPEG):
                        values = validate.validate_jpeg(flow.codec, data)
                    else:
                        raise NotImplementedError(
                            "Validation for {} codec doesn't seem to be implemented".format(
                                flow.codec
                            )
                        )
                ...
                # Save progress stats
                validate.save_training_progress(
                    training_summary, flow, nip_save_dir, quiet=True
                )
                ...
                if flow.is_trainable("nip"):
                    flow.nip.save_model(
                        os.path.join(model_directory), epoch, quiet=True
                    )

                if isinstance(flow.codec, compression.DCN) and flow.is_trainable("dcn"):
                    flow.codec.save_model(
                        os.path.join(model_directory), epoch, quiet=True
                    )
                ...
                if collect_memory_stats["ram"]:
                    ...
                    memory["cpu-resource"].append(
                        round(debugging.memory_usage_resource(), 1)
                    )
            ...
            if flow.is_trainable("dcn"):
                progress_stats["codec"] = flow.codec.performance["ssim"]["validation"][
                    -1
                ]
                progress_stats["H"] = flow.codec.performance["entropy"]["validation"][
                    -1
                ]
    ...
    if flow.is_trainable("nip"):
        values = validate.validate_nip(
            flow.nip, data, nip_save_dir, epoch=epoch, show_ref=True, loss_type="L2"
        )
        for metric, val_array in zip(["ssim", "psnr", "loss"], values):
            flow.nip.log_metric(metric, "validation", val_array)
    ...
    # Save model progress
    validate.save_training_progress(training_summary, flow, nip_save_dir)

    # Visualize current progress
    validate.visualize_manipulation_training(flow, epoch, nip_save_dir)
    ...
    if flow.is_trainable("dcn") and isinstance(flow.codec, compression.DCN):
        ...
        shutil.copyfile(
            os.path.join(
                flow._distribution["compression_params"]["dirname"], "progress.json"
            ),
