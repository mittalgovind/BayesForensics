...
class ManipulationClassification(object):
    def __init__(
        ...
        if distribution is None:
            self._distribution = {
                "downsampling": "pool:2",
                "compression": "jpeg",
                "compression_params": {"quality": 50, "codec": "soft"},
            }
        else:
            self._distribution = {}
        ...
        self.nip = getattr(pipelines, nip_model)(
            loss_metric=loss_metric, patch_size=raw_patch_size
        )
        ...
        with tf.name_scope("distribution"):
            ...
            if "sharpen" in manipulations_set:
                self._operations[
                    "sharpen"
                ] = lambda x, strength: tf_helpers.manipulation_sharpen(
                    x, strength, hsv=True
                )
            ...
            if "resample" in manipulations_set:
                self._operations[
                    "resample"
                ] = lambda x, strength: tf_helpers.manipulation_resample(x, strength)
                self._forensics_classes.append(
            ...
            if "gaussian" in manipulations_set:
                self._operations[
                    "gaussian"
                ] = lambda x, strength: tf_helpers.manipulation_gaussian(x, 5, strength)
                self._forensics_classes.append(
            ...
            if "awgn" in manipulations_set:
                self._operations[
                    "awgn"
                ] = lambda x, strength: tf_helpers.manipulation_awgn(x, strength / 255)
                self._forensics_classes.append(
            ...
            if "gamma" in manipulations_set:
                self._operations[
                    "gamma"
                ] = lambda x, strength: tf_helpers.manipulation_gamma(x, strength)
                self._forensics_classes.append(
            ...
            if "median" in manipulations_set:
                self._operations[
                    "median"
                ] = lambda x, strength: tf_helpers.manipulation_median(x, strength)
                self._forensics_classes.append(
        ...
        self.fan = forensics.FAN(
            n_classes=self.n_classes, patch_size=fan_input_patch, **fan_args
        )
    ...
    def run_rgb_to_probabilities(self, batch_Y):
        ...
        batch_C = self.run_compression(batch_c)
        probabilities = self.fan.process(batch_C)
        if not isinstance(probabilities, np.ndarray) and hasattr(
            probabilities, "numpy"
        ):
    ...
    def run_manipulations(self, batch_y, randomize=False, override=None):
        ...
        for name, op in self._operations.items():
            s = (
                override[name]
                if not randomize
                else np.random.uniform(*self._strengths_range[name])
            )
    ...
    def run_downsampling(self, batch_y):
        ...
        if self._distribution["downsampling"].startswith("pool"):
            imb_down = tf.nn.avg_pool(
                batch_y, [1, factor, factor, 1], [1, factor, factor, 1], "SAME"
            )

        elif self._distribution["downsampling"] == "bilinear":
            imb_down = tf.image.resize(
                batch_y,
                [tf.shape(batch_y)[1] // factor, tf.shape(batch_y)[1] // factor],
                method="bilinear",
            )
            ...
            raise ValueError(
                "Unsupported channel down-sampling {}".format(
    ...
    def run_compression(self, batch_y, return_entropy=False):
        if self._distribution["compression"] == "jpeg":
            ...
            raise ValueError(
                "Unsupported channel compression {}".format(
    ...
    def _batch_labels(self, batch_size):
        return np.concatenate(
            [x * np.ones((batch_size,), dtype=np.int32) for x in range(self.n_classes)]
        )
    ...
    def training_step(
        ...
        if any(np.sum(np.isnan(x)) > 0 for x in grads):
            raise RuntimeError(
                "∇ NaNs: {}".format(
                    {
                        p.name: np.mean(np.isnan(x))
                        for x, p in zip(grads, self._parameters)
                    }
                )
            )
    ...
    def summary_compact(self):
        return "{class_name}[{trainables}]: {nip} -> [{manips}] {pool}{codec}-> {fan}".format(
            ...
            pool=""
            if self._distribution["downsampling"] == "none"
            else "-> {} ".format(self._distribution["downsampling"]),
            codec=""
            if self.codec is None
            else "-> {} ".format(self.codec.summary_compact()),
            fan="FAN",
        )
    ...
    def details(self):
        ...
        out.append(
            "Input         : {} {}".format(
                self.nip.x.shape, "(rgb)" if self.nip.x.shape[-1] == 3 else "(raw)"
            )
        )
        out.append("Camera ISP    : {}".format(self.nip.summary()))
        out.append(
            "Manipulations : {} -> {}".format(self.n_classes, self._forensics_classes)
        )
