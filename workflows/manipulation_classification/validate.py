...
def validate_jpeg(jpeg, data, batch_size=1):
    ...
    if not isinstance(jpeg, JPEG):
        raise ValueError(
            "Codec needs to be as instance of {} but is {}".format(
                JPEG, jpeg.class_name
            )
        )
...
def validate_dcn(dcn, data, save_dir=False, epoch=0, show_ref=False):
    ...
    if save_dir is not None:
        ...
        fig = Figure(
            figsize=(20, 20 / images_x * images_y * (1 if not show_ref else 0.5))
        )

        for b in range(data.count_validation):
            ax = fig.add_subplot(images_y, images_x, b + 1)
            plots.image(
                np.concatenate((batch_x[b], batch_y[b]), axis=1)
                if show_ref
                else batch_y[b],
        ...
        fig.savefig(
            "{}/dcn_validation_{:05d}.jpg".format(save_dir, epoch),
            bbox_inches="tight",
            dpi=100,
            quality=90,
        )
...
def validate_nip(model, data, save_dir=False, epoch=0, show_ref=False, loss_type="L2"):
    ...
    for b in range(data.count_validation):
        ...
        if save_dir is not None:
            ax = fig.add_subplot(images_y, images_x, b + 1)
            plots.image(
                np.concatenate((reference, developed), axis=1)
                if show_ref
                else developed,
    ...
    if save_dir is not None:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        fig.savefig(
            "{}/nip_validation_{:05d}.jpg".format(save_dir, epoch),
            bbox_inches="tight",
            dpi=100,
            quality=90,
        )
...
def visualize_manipulation_training(flow, epoch, save_dir=None):
    ...
    if images_y == 3:
        ...
        ax.plot(
            helpers.stats.ma_conv(flow.codec.performance["entropy"]["validation"], 0)
        )
    ...
    if save_dir is not None:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        fig.savefig(
            "{}/manip_validation_{:05d}.jpg".format(save_dir, epoch),
            bbox_inches="tight",
            dpi=100,
        )
        del fig
