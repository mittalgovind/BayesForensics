...
class BayarStammSFP(BayesBaseModel):
    ...
    def __init__(self, method, n_classes, patch_size=None, n_filters=32,
                 n_fscale=2,
                 n_convolutions=4, kernel=5, dropout=0.0, use_gap=True,
                 n_dense=0, activation='leaky_relu', **kwargs):
        """
    ...
    def _create_model(self):
        ...
        for _ in range(self._h.n_convolutions):
            self._layers.append(tf.keras.layers.Conv2D(self._h.n_filters,
                                                       [self._h.kernel,
                                                        self._h.kernel],
                                                       padding='same',
                                                       activation=self.activation))
        ...
        self._layers.append(
            tf.keras.layers.Conv2D(int(self._h.n_filters), [1, 1],
                                   activation=self.activation))
        ...
        self._layers.append(tf.keras.layers.Dense(self._h.n_classes,
