...
def batch_training(
    ...
    if jpeg_quality is not None:
        if re.match("^[0-9]+$", jpeg_quality):
            ...
            raise FileNotFoundError(
                "Invalid JPEG quality: expecting a number or comma separated numbers & got: {}".format(
                    jpeg_quality
                )
            )
    ...
    if lambdas_nip is None or len(lambdas_nip) == 0:
        lambdas_nip = (
            [1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 0.1, 0.25, 0.5, 1]
            if "nip" in trainables
            else [0]
        )
    else:
    ...
    flow = manipulation_classification.ManipulationClassification(
        nip_model,
        manipulations,
        distribution,
        fan_args,
        trainables,
        raw_patch_size=training["patch_size"],
    )
    ...
    for camera_name in camera_names:
        ...
        if nip_model == "ONet":
            ...
            data_directory = os.path.join(
                root_directory, "raw", "training_data", camera_name
            )
            patch_mul = 2
            load = "xy"
        ...
        if not os.path.isdir(data_directory):
            logger.warning(
                "Training images not found in the target root directory - using default root as image source"
            )
        ...
        data = dataset.Dataset(
            ...
            load=load,
            val_rgb_patch_size=patch_mul * training["patch_size"],
            val_n_patches=training["val_n_patches"],
        )

        logger.info(
            "Training loop: {} repetitions / {} NIP lambdas {} / {} DCN lambdas {}".format(
        ...
        for rep in range(start_repetition, end_repetition):
            for lr in lambdas_nip:
                for lc in lambdas_dcn:
                    ...
                    train_manipulation_nip(
                        flow,
                        training,
                        data,
                        {"root": root_directory, "nip_snapshots": nip_directory},
                    )


def main():
    parser = argparse.ArgumentParser(
        description="NIP & FAN optimization for manipulation detection"
    )
    ...
    group.add_argument(
        ...
        dest="manipulations",
        action="store",
        default="sharpen,resample,gaussian,jpeg",
        help="comma-sep. list of manipulations (:strength), e.g., : {}".format(
            "sharpen:1,jpeg:80,resample,gaussian"
        ),
    )
    group.add_argument(
        "--fan",
    ...
    try:
        args.fan_args = (
            json.loads(args.fan_args.replace("'", '"'))
            if args.fan_args is not None
            else {}
        )
    ...
    batch_training(
        ...
        args.downsampling,
        patch=args.patch // 2,
        fan_args=args.fan_args,
        use_pretrained=not args.from_scratch,
        start_repetition=args.start,
        end_repetition=args.end,
        n_epochs=args.epochs,
        nip_directory=args.nip_directory,
        split=args.split,
        lambdas_nip=args.lambdas_nip,
        lambdas_dcn=args.lambdas_dcn,
    )
