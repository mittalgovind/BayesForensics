...
class Dataset(object):
    ...
    def summary(self):
        ...
        return (
            ...
            f"{self.count_training} train. images ({tf.cast(tf.reduce_prod(self.train_image_shape_rgb[:2]), tf.float32) / 1e6:.1f} Mpx) "
    ...
    def get_training_generator(self, discard="flat", **kwargs):
        ...
        for batch in self.data["training"]["y"]:
            ...
            images = self.preprocess_batch(batch, training=True, **kwargs)
            yield images
    ...
    def get_validation_generator(self, **kwargs):
        ...
        for batch in self.data["validation"]["y"]:
            images = self.preprocess_batch(batch, training=False, **kwargs)
            yield images
    ...
    def get_calibration_generator(self, **kwargs):
        ...
        for batch in self.data["calibration"]["y"]:
            images = self.preprocess_batch(batch, **kwargs)
            yield images
...
class LabeledDataset(object):
    ...
    def __init__(
        ...
        c_images = v_images if calibrate else 0
        self.data = {"training": {}, "validation": {}, "calibration": {}}
        self.files = {"training": [], "validation": [], "calibration": []}
        self.batch_size = batch_size
        self._loaded_data = load
        self._data_directory = data_dir
        self._counts = (
        ...
        if data_dir:
            # Discover images files to sample from.
            if labels is None:
                self.labels = [l for l in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, l)) and not l.startswith('.')]
              
            n_per_label = n_images
            if n_images >= 1:
                n_per_label = int(n_images / len(self.labels))
            v_per_label = v_images
            if v_images >= 1:
                v_per_label = int(v_images / len(self.labels))
            c_per_label = c_images
            if c_images >= 1:
                c_per_label = int(c_images / len(self.labels))
            
            for l in self.labels:
                label_dir = os.path.join(data_dir, l)
                
                t_files, v_files, c_files = loading.discover_images(
                    label_dir, randomize=seed, n_images=n_per_label,
                    v_images=v_per_label, c_images=c_per_label,
                )
                
                for f in t_files:
                    self.files["training"].append(os.path.join(l, f))
                for f in v_files:
                    self.files["validation"].append(os.path.join(l, f))
                for f in c_files:
                    self.files["calibration"].append(os.path.join(l, f))
                    
            self.t_labels = np.zeros((len(self.files["training"]), ))
            self.v_labels = np.zeros((len(self.files["validation"]), ))
            self.c_labels = np.zeros((len(self.files["calibration"]), ))
            for i in range(len(self.labels)):
                n_per_label = int(len(self.files["training"]) / len(self.labels))
                v_per_label = int(len(self.files["validation"]) / len(self.labels))
                c_per_label = int(len(self.files["calibration"]) / len(self.labels))
                
                for j in range(n_per_label):
                    self.t_labels[(i * n_per_label) + j] = i
                
                for j in range(v_per_label):
                    self.v_labels[(i * v_per_label) + j] = i
                
                for j in range(c_per_label):
                    self.c_labels[(i * c_per_label) + j] = i
                    
            self._counts = (
                len(self.files["training"]) * train_n_patches,
                len(self.files["validation"]) * val_n_patches,
                len(self.files["calibration"]),
                val_n_patches,
            )
        ...
        if "x" in load:
            if n_images > 0:
                ...
                self.data["training"][
                    "x"] = tf.data.Dataset.from_tensor_slices(
                    (self.data["training"]["x"], self.t_labels)).batch(batch_size,
                                                      drop_remainder=True)
            self.data["validation"]["x"] = tf.math.divide(
                self.data["validation"]["x"], 2 ** 16 - 1)
            self.data["validation"]["x"] = tf.data.Dataset.from_tensor_slices(
                (self.data["validation"]["x"], self.v_labels)).batch(batch_size,
                                                    drop_remainder=True)
            if calibrate:
                self.data["calibration"]["x"] = tf.math.divide(
                    self.data["calibration"]["x"], 2 ** 16 - 1)
                self.data["calibration"][
                    "x"] = tf.data.Dataset.from_tensor_slices(
                    (self.data["calibration"]["x"], self.c_labels)).batch(batch_size,
                                                         drop_remainder=True)
        if "y" in load:
            if n_images > 0:
                if self.preloading_train:
                ...
                self.data["training"][
                    "y"] = tf.data.Dataset.from_tensor_slices(
                    (self.data["training"]["y"], self.t_labels)).shuffle(self.count_training).batch(batch_size,
                                                      drop_remainder=True)

            self.data["validation"]["y"] = tf.math.divide(
                self.data["validation"]["y"], 2 ** 8 - 1)
            self.data["validation"]["y"] = tf.data.Dataset.from_tensor_slices(
                (self.data["validation"]["y"], self.v_labels)).shuffle(self.count_validation).batch(batch_size,
                                                    drop_remainder=True)
            if calibrate:
                self.data["calibration"]["y"] = tf.math.divide(
                    self.data["calibration"]["y"], 2 ** 8 - 1)
                self.data["calibration"][
                    "y"] = tf.data.Dataset.from_tensor_slices(
                    (self.data["calibration"]["y"], self.c_labels)).shuffle(self.count_calibration).batch(batch_size,
                                                         drop_remainder=True)
    ...
    def train_image_shape_rgb(self):
        """returns shape - (patch_size, patch_size, channels)"""
        if "y" in self._loaded_data:
            image_shape = self.data['training']['y'].element_spec[0].shape
        else:
            image_shape = self.data['training']['x'].element_spec[0].shape
        return image_shape[1:]
    ...
    def preprocess_batch(self, batch, labels, **kwargs):
        """
        Override this method to return the processed batch and its labels.
        """
        return batch, labels
    ...
    def get_training_generator(self, discard="flat", **kwargs):
        ...
        dp = tf.data.Dataset.from_generator(lambda: data.get_training_generator(batch_size, rgb_patch_size, discard),
            output_types=len(self._loaded_data) * (tf.float32, ))
        """
        for batch, labels in self.data["training"]["y"]:
            if not self.preloading_train:
                batch = self.sample_patches(batch, discard, **kwargs)
            images, labels = self.preprocess_batch(batch, labels, training=True, **kwargs)
            yield images, labels
    ...
    def get_validation_generator(self, **kwargs):
        ...
        dp = tf.data.Dataset.from_generator(lambda: data.get_validation_generator(batch_size),
            output_types=len(self._loaded_data) * (tf.float32, ))
        """
        for batch, labels in self.data["validation"]["y"]:
            images, labels = self.preprocess_batch(batch, labels, training=False, **kwargs)
            yield images, labels
    ...
    def get_calibration_generator(self, **kwargs):
        ...
        dp = tf.data.Dataset.from_generator(lambda: data.get_calibration_generator(batch_size),
            output_types=len(self._loaded_data) * (tf.float32, ))
        """
        for batch, labels in self.data["calibration"]["y"]:
            images, labels = self.preprocess_batch(batch, labels, training=False, **kwargs)
            yield images, labels
