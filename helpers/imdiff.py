...
def compare_images_ab_ref(img_ref, img_a, img_b, labels=None, extras=False, fig=None):
    ...
    label_a = "(A) {}: {:.1f} dB / {:.3f}".format(
        labels[1], metrics.psnr(img_ref, img_a), metrics.ssim(img_ref, img_a)
    )
    plots.image(img_a, label_a, axes=axes[1])

    label_b = "(B) {}: {:.1f} dB / {:.3f}".format(
        labels[2], metrics.psnr(img_ref, img_b), metrics.ssim(img_ref, img_b)
    )
...
def compare_batches(
    batch_a, batch_b, labels=None, fig=None, figwidth=4, nrows=3, transpose=False
):
    ...
    fig, axes = plots.sub(
        n_images * nrows, figwidth, ncols=n_images, fig=fig, transpose=transpose
    )
    ...
    for i, (img_a, img_b) in enumerate(zip(batch_a, batch_b)):
        ...
        if nrows > 2:
            plots.image(
                image.normalize(diff_ab),
                f"A - B: mean abs {diff_mean:.3f}",
                axes=axes[i + n_images * 2],
            )

        if nrows > 3:
            plots.image(
                image.normalize(diff_abs, 0.1),
                f"|A - B|: mean abs {diff_mean:.3f}",
                axes=axes[i + n_images * 3],
            )
