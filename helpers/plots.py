...
def thumbnails(images, ncols=0, columnwise=False, cell_size=None, pad=0):
    ...
    output = np.zeros(
        (
            size[0] * (img_size[0] + 2 * pad),
            size[1] * (img_size[1] + 2 * pad),
            img_size[2],
        )
    )
    ...
    for r in range(n_images):
        ...
        output[
            by * (img_size[0] + 2 * pad) : (by + 1) * (img_size[0] + 2 * pad),
            bx * (img_size[1] + 2 * pad) : (bx + 1) * (img_size[1] + 2 * pad),
            :,
        ] = current
...
def _imarray(
    ...
    if n_images > 128:
        raise RuntimeError(
            "The number of subplots exceeds reasonable limits ({})!".format(n_images)
        )
    ...
    if titles is not None and len(titles) != n_images:
        raise ValueError(
            "Provided titles ({}) do not match the number of images ({})!".format(
                len(titles), n_images
            )
        )
    ...
    for n in range(n_images):
        ...
        image(
            fetch_hook(img, n),
            titles[n] if titles is not None else None,
            axes=ax,
            cmap=cmap,
            vrange=vrange,
        )
...
def image(x, label=None, *, axes=None, cmap="gray", vrange=None):
    ...
    if vrange is None:
        ...
        raise ValueError(
            "Invalid vrange - supported types: None, 2-elem tuple & boolean"
        )
...
def sub(n_plots, figwidth=6, figheight=None, ncols=-1, fig=None, transpose=False):
    ...
    if transpose:
        ...
        axes_flat = [
            axes_flat[j * subplot_x + i]
            for i, j in product(range(subplot_x), range(subplot_y))
        ]
...
def progress(
    ...
    for ri, r in enumerate(results):
        ...
        axes.plot(
            xr, ma, color or f"C{ri}-", label=f"{k}:{r} ({utils.format_number(ma[-1])})"
        )

        # Change scale to log if requested
        auto_switch_to_log = (
            log == "auto" and np.std(v[r][-n_hist:]) / (max(v[r]) - min(v[r])) < 0.02
        )
...
def perf(
    ...
    if any(not isinstance(v, dict) for v in training_progress.values()):
        auto = {
            k: {"auto": v} for k, v in training_progress.items() if utils.is_vector(v)
        }
        training_progress = {
            k: v for k, v in training_progress.items() if isinstance(v, dict)
        }
    ...
    if len(active) == 0:
        raise ValueError(
            'No valid plots! Missing training/validation data? Use results=["training"] to select.'
        )
...
def boxplots(samples, labels, axes=None, guides=None, xlabel=None):
    ...
    bp = axes.boxplot(
        [x.ravel() for x in samples], vert=False, notch="True", patch_artist=True
    )
...
def hist(
    ...
    for si, spls in enumerate(samples):
        ...
        h1 = axes.hist(
            spls.ravel(), h_bins, color=color, alpha=alpha, density=True, label=label
        )
...
def detection(
    ...
    bin_accuracy, thr_id = stats.detection_accuracy(
        positive, negative, cc, return_index=True
    )
    ...
    h1 = axes.hist(
        positive.ravel(), h_bins, color="g", alpha=0.4, density=True, label="positive"
    )
    h2 = axes.hist(
        negative.ravel(), h_bins, color="r", alpha=0.4, density=True, label="negative"
    )
    if reference is not None:
        h3 = axes.hist(
            reference.ravel(),
    ...
    if utils.is_number(guides):
        if guides == 2:
            ...
            axes.plot(
                [cc[thr_id], cc[thr_id]], [0, max(h1[0][thr_id], 0.05 * h_max)], "k:"
            )
        elif guides == 1:
    ...
    axes.set_title(
        title.replace("()", f"acc. {bin_accuracy:.2f}, tpr @ 1\\%far={tpr:.2f}")
    )
...
def roc(matching, non_matching, bins=1000, axes=None, label=None, plot_guides=True):
    ...
    label = (
        f"{label} : tpr={tpr_at_1pp_fpr:.2f} auc={auc:.2f}"
...
def intervals_bulk(x, y, p=10):
    ...
    for i, (k, v) in enumerate(y.items()):
        ...
        axes[i].fill_between(
            xv,
            np.percentile(v, p, axis=0),
            np.percentile(v, 100 - p, axis=0),
            alpha=0.2,
            edgecolor="#1B2ACC",
            facecolor="#089FFF",
        )
...
def intervals(x, y, p=10, xlabel=None, ylabel=None, style=".-", axes=None, label=None):
    ...
    axes.fill_between(
        x,
        np.percentile(y, p, axis=0),
        np.percentile(y, 100 - p, axis=0),
        alpha=0.2,
        edgecolor=color,
        facecolor=color,
    )
...
def correlation(
    ...
    if marginals:
        ...
        axes.bar(
            x_bins,
            bottom=yy[1],
            height=0.1 * np.abs(yy[1] - yy[0]) * x_hist,
            zorder=-1,
            clip_on=False,
            alpha=0.5,
        ...
        axes.barh(
            y_bins,
            left=xx[1],
            width=0.1 * np.abs(xx[1] - xx[0]) * y_hist,
            zorder=-1,
            clip_on=False,
            alpha=0.5,
            height=y_bins[1] - y_bins[0],
        )
        axes.set_xlim(xx)
