...
def setup_logging(filename=None, long_date=False, level="INFO"):
    ...
    if long_date:
        ...
        log_format = (
            "<green>{time:HH:mm:ss}</> | <lvl>{level}</> | <lvl>{message}</lvl>"
        )
...
def format_sequence_rle(numbers, sep="-"):
    ...
    for i in range(1, len(numbers)):
        if numbers[i - 1] != numbers[i]:
            items.append(
                f"{numbers[i - 1]}x{counter}" if counter > 1 else f"{numbers[i - 1]}"
            )
            counter = 1
        else:
            counter += 1
...
def printd(d, filters=None, indent=2, level=1):
    ...
    if isinstance(filters, str):
        raise ValueError(
            "The filters should be a collection of strings (for each recursion level)"
        )
    ...
    for k, v in d.items():
        ...
        if isinstance(v, dict):
            ...
            if v.ndim == 0:
                ...
                print(
                    f"array {v.shape} ∈ [{format_number(v.min())}, {format_number(v.max())}]"
                )
            ...
            if len(v) < 5:
                ...
                print(
                    f"tuple of {len(v)} items: ({format_number(v[0])}, ..., {format_number(v[-1])})"
                )
...
def factory(spec):
    ...
    if isinstance(spec, str):
        ...
        if "args" in spec:
            ...
            raise ValueError(
                'Invalid model definition! Was expecting either "args" or "checkpoint" keys!'
            )
