    ...
    def _validate_specs(specs):
        for key, spec in specs.items():
            if type(spec) is not tuple and len(spec) != 3:
                raise ValueError(
                    "Invalid parameter specification for key {} - expected tuple of length 3".format(
                        key
                    )
                )
            ...
            if spec[1] is str and not any(
                type(spec[2]) is s for s in [str, set, types.FunctionType]
            ):
                raise ValueError(
                    f"Error at key={key}: String data types can be validated by a regex (string), enum (set) or custom function"
                )

            if utils.is_numeric_type(spec[1]) and not any(
                type(spec[2]) is s for s in [tuple, set]
            ):
                raise ValueError(
                    f"Error at key={key}: Numeric data types can be validated by a range (2-elem tuple), or enum (set)"
                )
            ...
            if key in self._specs:
                ...
                if value is not None:
                    ...
                    if utils.is_number(value) and np.isnan(value):
                        raise ValueError(
                            "Invalid value {} for attribute {}".format(value, key)
                        )
                    ...
                    if validation is not None:
                        ...
                        if type(validation) == tuple and len(validation) == 2:
                            if validation[0] is not None and candidate < validation[0]:
                                raise ValueError(
                                    "{}: {} fails minimum validation check >= {}!".format(
                                        key, candidate, validation[0]
                                    )
                                )
                            if validation[1] is not None and candidate > validation[1]:
                                raise ValueError(
                                    "{}: {} fails maximum validation check (<= {})!".format(
                                        key, candidate, validation[1]
                                    )
                                )
                        ...
                        if type(validation) == set:
                            if candidate not in validation:
                                raise ValueError(
                                    "{}: {} is not an allowed value ({})!".format(
                        ...
                        if callable(validation):
                            if not validation(candidate):
                                raise ValueError(
                                    "{}: {} failed custom validation check!".format(
                                        key, candidate
                                    )
                                )
