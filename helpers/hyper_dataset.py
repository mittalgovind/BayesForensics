...
class Dataset(object):
    def __init__(
        ...
        def next_validation_batch(self, batch_id, batch_size):
            ...
            bx = (
                tf.cast(
                    tf.zeros((batch_size, rgb_patch // 2, rgb_patch // 2, 4)),
                    tf.float32)
            ...
            for b in range(batch_size):
                if has_raw:
                    bx[b] = tf.cast(
                        self.data["validation"]["x"][
                            batch_id * batch_size + b],
                        tf.float32) / (2 ** 16 - 1)
                if has_rgb:
        ...
        def shapes(self):
            ...
            for k in self._loaded_data:
                ...
                stats["validation/{}".format(k)] = self.data["validation"][
                    k].shape
        ...
        def get_training_generator(self, batch_size, patch_size,
                                   discard="flat",
                                   **kwargs):
            """
        ...
        def get_training_pipeline(self, batch_size, rgb_patch_size,
            ...
            types = (
                tf.float32,
                tf.float32) if self.is_raw_and_rgb() else tf.float32
            shapes = (
