...
def match_jpeg(model, batch_x, axes=None, match="ssim"):
    ...
    try:
        jpeg_quality = jpeg_helpers.match_quality(
            batch_x.squeeze(), target, match=match
        )
    except:
        if match == "ssim":
        ...
        print(
            "WARNING Could not find a matching JPEG quality factor - guessing {}".format(
                jpeg_quality
            )
        )

    # Compress using JPEG
    batch_j, bytes_jpeg = jpeg_helpers.compress_batch(
        batch_x[0], jpeg_quality, effective=True
    )
    ...
    print(
        "Code-book size  : {} elements from {} to {}".format(
    ...
    print(
        "PPF Naive       : {:,.0f} --> {:,.0f} bytes [{} bits per element]".format(
    ...
    print(
        "PPF Theoretical : {:,.0f} bytes ({:.2f} bpp)".format(
            np.prod(batch_z.shape) * entropy / 8,
            np.prod(batch_z.shape) * entropy / np.prod(batch_x.shape[1:-1]),
        )
    )
    print(
        "FSE Coded       : {:,} bytes ({:.2f} bpp) --> ssim: {:.3f}".format(
            bytes_dcn, bpp_dcn, ssim_dcn
        )
    )
    print(
        "JPEG (Q={:2d})     : {:,} bytes ({:0.2f} bpp) --> ssim: {:.3f} // effective size disregarding JPEG headers".format(
            jpeg_quality, bytes_jpeg, bpp_jpg, ssim_jpeg
        )
    )
    ...
    plots.image(
        batch_y, "DCN ssim:{:.2f} bpp:{:.2f}".format(ssim_dcn, bpp_dcn), axes=axes[1]
    )
    plots.image(
        batch_j,
        "JPEG {} ssim:{:.2f} bpp:{:.2f}".format(jpeg_quality, ssim_jpeg, bpp_jpg),
        axes=axes[2],
    )
    ...
    plots.image(
        helpers.image.crop_middle(batch_x, crop_size),
        "Original crop ({0}x{0})".format(crop_size),
        axes=axes[3],
    )
    plots.image(
        helpers.image.crop_middle(batch_y, crop_size),
        "DCN crop ({0}x{0})".format(crop_size),
        axes=axes[4],
    )
    plots.image(
        helpers.image.crop_middle(batch_j, crop_size),
...
def show_example(model, batch_x):
    ...
    hist_emp = np.histogram(batch_z.reshape((-1,)), bins=bin_boundaries, density=True)[
        0
    ]
    ...
    thumbs_few = (255 * plots.thumbnails(thumbs_pairs_few, ncols=len(batch_x))).astype(
        np.uint8
    )

    ssim_values = [
        metrics.ssim(batch_x[i], batch_y[i]).mean() for i in range(len(batch_x))
    ]

    plots.image(
        thumbs_few,
        "Sample reconstructions, ssim={:.3f}".format(np.mean(ssim_values)),
        axes=axes[1],
    )
...
def main():
    ...
    parser.add_argument(
        "plot", help="Plot type ({})".format(", ".join(supported_plots))
    )
    parser.add_argument(
        "--data",
        dest="data",
    ...
    parser.add_argument(
        "--dcn", dest="dcn", action="store", help="directory with a trained DCN model"
    )
