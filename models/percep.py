...
def get_loss(img0_feats, img1_feats):
    ...
    for init_img_content_layer, content_img_content_layer in zip(img0_feats, img1_feats):
...
def percep_dist(vgg_model, imgbatch0, imgbatch1):
        ...
        for each_image_idx in range(len(batch0_vggfeats)):
            img0_feats = [content_layer[0] for content_layer in batch0_vggfeats[each_image_idx][params.num_style_layers:]]
            img1_feats = [content_layer[0] for content_layer in batch1_vggfeats[each_image_idx][params.num_style_layers:]]
