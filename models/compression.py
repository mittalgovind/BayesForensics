...
class DCN(TFModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.discrete_latent = DiscreteLatent(
            self._h.rounding, latent_bpf=self._h.latent_bpf
        )
        ...
        if loss_metric == "L2":
            ...
            def mse_entropy(image_target, image_compressed, entropy):
                return (
                    tf.nn.l2_loss(image_target - image_compressed)
                    + self._h.entropy_weight * entropy
                )

            self.loss = mse_entropy
        else:
            raise NotImplementedError(
                "Loss metric {} not supported yet.".format(loss_metric)
            )
    ...
    def compress(self, batch_x):
        ...
        return self._encoder(
            np.expand_dims(batch_x, axis=0) if batch_x.ndim == 3 else batch_x
        )[0]

    def decompress(self, batch_z):
        """ Decompress a batch of images from their quantized latent representations. """
        return self._decoder(
            np.expand_dims(batch_z, axis=0) if batch_z.ndim == 3 else batch_z
        )
    ...
    def training_step(self, batch_x, learning_rate=None):
        ...
        return {
            "loss": np.sqrt(
                2 * loss
            ),  # The L2 loss in TF is computed differently (half of non-square rooted norm)
            "ssim": ssim,
            "entropy": entropy,
        }
    ...
    def model_code(self):
        if not hasattr(self, "n_latent"):
            raise ValueError(
                "The model does not report the latent space dimensionality."
            )
...
class TwitterDCN(DCN):
    ...
    def construct_model(self, n_features=32, activation="leaky_relu"):
        ...
        net = tf.keras.layers.Conv2D(64, 5, 2, padding="SAME", activation=activation)(
            net
        )
        net = tf.keras.layers.Conv2D(128, 5, 2, padding="SAME", activation=None)(net)

        net_relu = tf.nn.leaky_relu(net)
        resnet = tf.keras.layers.Conv2D(
            128, 3, 1, padding="SAME", activation=activation
        )(net_relu)
        resnet = tf.keras.layers.Conv2D(128, 3, 1, padding="SAME", activation=None)(
            resnet
        )
        ...
        net = tf.keras.layers.Conv2D(
            self._h.n_features, 5, 2, padding="SAME", activation=None
        )(net)
        ...
        self.latent_input = tf.keras.Input(
            dtype=tf.float32, shape=self.latent.shape[1:]
        )

        inet = tf.keras.layers.Conv2D(512, 3, 1, padding="SAME", activation=None)(
            self.latent_input
        )
        ...
        inet = tf.keras.layers.Conv2D(256, 3, 1, padding="SAME", activation=activation)(
            inet
        )
        inet = tf.nn.depth_to_space(inet, 2)
        ...
        self._encoder = tf.keras.Model(
            inputs=self.x, outputs=[self.latent, self.entropy], name="encoder"
        )
        self._decoder = tf.keras.Model(
            inputs=self.latent_input, outputs=self.y, name="decoder"
        )
        ...
        self._model = tf.keras.Model(
            inputs=self.x, outputs=[self._decoder(latent), entropy], name="codec"
        )
    ...
    def model_code(self):
        ...
        parameter_summary.append(
            f"Q+{self._h.latent_bpf}bpf"
            if self._h.train_codebook
            else f"Q-{self._h.latent_bpf}bpf"
        )
