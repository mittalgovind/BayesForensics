...
class Quantization(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if self.trainable:
            ...
            self.codebook = tf.constant(
                np.arange(qmin, qmax + 1),
                shape=(1, 2 ** self.latent_bpf),
                dtype=tf.float32,
            )
        ...
        if self.rounding == "round":
            ...
            if self.v <= 0:
                ...
                weights = tf.exp(
                    -self.gamma
                    * tf.pow(
                        tf.cast(values, dtype=prec_dtype)
                        - tf.cast(self.codebook, dtype=prec_dtype),
                        2,
                    )
                )
            else:
                # t-Student soft quantization
                dff = tf.cast(values, dtype=prec_dtype) - tf.cast(
...
class DiscreteLatent(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self.quantization = Quantization(
            rounding, v, gamma, latent_bpf, trainable_codebook
        )
    ...
    def call(self, inputs):
        ...
        entropy_ = tf_helpers.entropy(
            latent, self.quantization.codebook, self.v, self.gamma
        )[0]
    ...
    def __init__(self, c_filters, kernel, activation, residual, **kwargs):
        ...
        if residual:
            ...
            self._alpha = self.add_weight(
                "alpha", initializer=tf.constant_initializer(0.1)
            )
        else:
            self._bilinear = None
        ...
        for n_filters in c_filters:
            self._layers.append(
                tf.keras.layers.Conv2D(
                    n_filters, kernel, 1, "same", activation=activation
                )
            )
