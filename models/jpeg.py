...
def is_valid_quality(quality):
    if is_number(quality) and 1 <= quality <= 100:
        ...
        hasattr(quality, "__getitem__")
        and len(quality) > 1
        and all((1 <= x <= 100) for x in quality)
    ):
        return True
    return False
...
class DifferentiableJPEG(tf.keras.Model):
    def __init__(
        ...
        if quality is not None and not is_valid_quality(quality):
            raise ValueError(
                "Invalid JPEG quality: requires int in [1,100] or an iterable with least 2 such numbers"
            )

        # Sanitize inputs
        if rounding_approximation is not None and rounding_approximation not in [
            "sin",
            "harmonic",
            "soft",
        ]:
            raise ValueError(
                "Unsupported rounding approximation: {}".format(rounding_approximation)
            )
        ...
        if trainable:
            q_mtx_luma_init = (
                ...
                else jpeg_qtable(quality, 0)
            )
            q_mtx_chroma_init = (
                np.ones((8, 8), dtype=np.float32)
                if not is_number(quality)
                else jpeg_qtable(quality, 1)
            )
            self._q_mtx_luma = self.add_weight(
                "Q_mtx_luma",
                [8, 8],
                dtype=tf.float32,
                initializer=tf.constant_initializer(q_mtx_luma_init),
            )
            self._q_mtx_chroma = self.add_weight(
                "Q_mtx_chroma",
                [8, 8],
                dtype=tf.float32,
                initializer=tf.constant_initializer(q_mtx_chroma_init),
            )
        else:
            self._q_mtx_luma = (
                np.ones((8, 8), dtype=np.float32)
        ...
        self._color_I = np.array(
            [
                [-1.402 * 128, 1, 0, 1.402],
                [1.058272 * 128, 1, -0.344136, -0.714136],
                [-1.772 * 128, 1, 1.772, 0],
            ],
            dtype=np.float32,
        )
    ...
    def uncompiled_call(self, inputs):
        ...
        with tf.name_scope("jpeg"):
            ...
            with tf.name_scope("blocking"):
                ...
                p = tf.reshape(
                    p, [-1, block_size, block_size, tf.shape(p)[2] * tf.shape(p)[3]]
                )
            ...
            with tf.name_scope("dct"):
                Xi = tf.matmul(
                    tf.tile(
                        tf.expand_dims(self._dct_F, axis=0), [tf.shape(r)[0], 1, 1]
                    ),
                    r,
                )
                X = tf.matmul(
                    Xi,
                    tf.tile(
                        tf.expand_dims(self._dct_I, axis=0), [tf.shape(r)[0], 1, 1]
                    ),
                )
            ...
            with tf.name_scope("quantization"):
                ...
                Qc = tf.tile(
                    tf.expand_dims(self._q_mtx_chroma, axis=0),
                    [2 * (tf.shape(p)[-1]), 1, 1],
                )
            ...
            with tf.name_scope("rev-blocking"):
                ...
                xi = tf.reshape(
                    xi, [3 * tf.shape(inputs)[0], -1, xi.shape[1], xi.shape[2]]
                )
                ...
                q = tf.reshape(
                    ...
                    [
                        -1,
                        tf.shape(xi)[1] * tf.shape(xi)[2],
                        tf.shape(inputs)[1] // block_size,
            ...
            with tf.name_scope("ycbcr_to_rgb"):
                ...
                y = tf.nn.conv2d(
                    qc,
                    tf.reshape(tf.transpose(self._color_I), [1, 1, 4, 3]),
                    [1, 1, 1, 1],
                    "SAME",
                )
                y = y / 255.0
...
class JPEG(TFModel):
    ...
    def process(self, batch_x, quality=None, return_entropy=False):
        ...
        if self._model is None:
            ...
            if quality != self.quality:
                old_q_luma, old_q_chroma = (
                    self._model._q_mtx_luma,
                    self._model._q_mtx_chroma,
                )
            ...
            if quality != self.quality:
                self._model._q_mtx_luma, self._model._q_mtx_chroma = (
                    old_q_luma,
                    old_q_chroma,
                )
    ...
    def __repr__(self):
        if self._model is not None:
            return 'JPEG(quality={},codec="{}",trainable={})'.format(
                self.quality, self.codec, self._model.trainable
            )
        else:
