...
def run_test(test_name, config, args):
    ...
    code = utils.shell(
        config["command"].format(cam=args.camera, root=args.root_dir),
        log_path,
        verbosity=1,
    )
    print("\n  Exit code: {}\n".format(code))
    ...
    for filename in config["files"]:
        status = os.path.isfile(
            os.path.join(args.root_dir, filename.format(args.camera))
        )
        print(
            "    {:84s} [{}]".format(
                filename.format(args.camera), OK_STR if status else MISS_STR
            )
        )

    # Check performance
    training_log = os.path.join(
        args.root_dir, config["performance"]["file"].format(args.camera)
    )
    ...
    with open(training_log) as f:
        ...
        for key, expected_value in config["performance"]["values"].items():
            ...
            print(
                "    {:70s} {:5.2f} > {:5.2f} [{}]".format(
                    key,
                    obtained_value,
                    expected_value,
                    OK_STR if obtained_value > expected_value else FAIL_STR,
                )
            )
...
def main():
    ...
    parser.add_argument(
        "--cam", dest="camera", action="store", help="camera", default="D90"
    )
    parser.add_argument(
        "--dir",
        dest="root_dir",
        action="store",
        default="/tmp/neural-imaging",
