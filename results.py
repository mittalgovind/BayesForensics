...
def display_results(args):
    ...
    if plot in ["ssim", "psnr", "accuracy"]:
        ...
        df = results_data.manipulation_metrics(
            args.nips, args.cameras, root_dir=args.dir
        )
    ...
    if plot == "progress":
        ...
        for cam in args.cameras:
            ...
            nip_models = args.nips or fsutil.listdir(
            ...
            for nip in nip_models:
                ...
                if args.regularization:
                    ...
                    if len(reg_list) > 4:
                        ...
                        print(
                            "! warning - too many experiments to show - sampling: {}".format(
                                reg_list
                            )
                        )

                for reg in reg_list:
                    for r in fsutil.listdir(
                        os.path.join(reg_path, reg), "[0-9]+", dirs_only=True
                    ):
    ...
    if plot == "auto":
        ...
        for i in components:
            ...
            if components.iloc[0, i].endswith("Net"):
                ...
                [re.match("^[0-9]{2,3}$", x) for x in components.iloc[:, i].unique()]
            ):
        ...
        uniques = [
            len(df[guessed_names["scenario:{}".format(i)]].unique()) for i in components
        ]
        ...
        sns.catplot(
            x=guessed_names["scenario:{}".format(x_feature)],
            y="accuracy",
            data=df,
            kind="box",
            **mapping
    ...
    raise RuntimeError(
        "No plot matched! Available plots {}".format(", ".join(supported_plots))
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Show results from NIP & FAN optimization"
    )
    parser.add_argument(
        "plot", help="Plot type ({})".format(", ".join(supported_plots))
    )
    parser.add_argument(
        "--nip", dest="nips", action="append", help="the NIP model (INet, UNet, DNet)"
    )
    parser.add_argument(
        "--cam",
        dest="cameras",
