...
def compare_nips(
    ...
    if isinstance(image, int) and camera is not None:
        ...
        sample_x, sample_y = (
            data["x"].astype(np.float32) / (2 ** 16 - 1),
            data["y"].astype(np.float32) / (2 ** 8 - 1),
        )
    ...
    if isinstance(model_a, pipelines.ClassicISP):
        print(
            "Configuring ISP-A to CFA: {} & sRGB {}".format(cfa, srgb.round(2).tolist())
        )
    ...
    if isinstance(model_b, pipelines.ClassicISP):
        print(
            "Configuring ISP-B to CFA: {} & sRGB {}".format(cfa, srgb.round(2).tolist())
        )
    ...
    if patch_size > 0:
        ...
        sample_y = sample_y[
            :, 2 * yy : 2 * (yy + patch_size), 2 * xx : 2 * (xx + patch_size), :
        ]
        sample_ya = sample_ya[
            :, 2 * yy : 2 * (yy + patch_size), 2 * xx : 2 * (xx + patch_size), :
        ]
    ...
    # Plot images
    fig = imdiff.compare_ab_ref(
        sample_y, sample_ya, sample_yb, fig=plt.figure(), extras=extras
    )
    ...
    if output_dir is not None:
        ...
        dcomp = [
            x for x in fsutil.split(model_b_dirname) if re.match("(ln-.*|[0-9]{3})", x)
        ]
        tikz_save(
            "{}/examples_{}_{}_{}_{}.tex".format(
                output_dir, camera, image, model_a_dirname, model_b_dirname
            ),
            figureheight="8cm",
            figurewidth="8cm",
            strict=False,
...
def main():
    ...
    parser.add_argument(
        ...
        dest="ha",
        default=None,
        help="Set hyper-parameters / override CSV settings for model A (JSON string)",
    )
    parser.add_argument(
        "--hb",
        dest="hb",
    ...
    try:
        if args.ha is not None:
            args.ha = json.loads(args.ha.replace("'", '"'))
    ...
    try:
        if args.hb is not None:
            args.hb = json.loads(args.hb.replace("'", '"'))
    except json.decoder.JSONDecodeError:
