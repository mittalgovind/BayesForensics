...
def test_quality(image, rounding_approximation=None, n_quality_levels=91):
    ...
    quality_levels = np.unique(
        np.round(np.linspace(10, 100, n_quality_levels)).astype(np.int32)
    ).tolist()
    ...
    for jpeg_quality in quality_levels:
        ...
        io.imwrite(
            "/tmp/patch.jpg",
            (batch_x.squeeze()).astype(np.uint8),
            quality=jpeg_quality,
            subsampling="4:4:4",
        )
    ...
    if rounding_approximation is None:
        plt.title(
            "dJPEG vs libJPEG quality (with standard rounding)".format(
                rounding_approximation
            )
        )
    else:
        plt.title(
            "dJPEG vs libJPEG quality (with {} rounding approx.)".format(
                rounding_approximation
            )
        )
...
def main():
    ...
    parser.add_argument("--image", dest="image", action="store", help="test image path")
    parser.add_argument(
        "--patch",
        dest="patch_size",
        action="store",
    ...
    if args.mode == "output":
        test_output(
            image, jpeg_quality=int(args.quality), rounding_approximation=args.round
        )

    elif args.mode == "quality":
        test_quality(
            image, n_quality_levels=int(args.quality), rounding_approximation=args.round
        )
