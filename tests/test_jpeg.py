...
def data():
    data = dataset.Dataset('native12k', load='y', n_images=0, v_images=50, val_n_patches=4)
    return data
...
def test_compare_soft_libjpeg(data, correlation_threshold=0.97):
    ...
    assert np.all(q_correlations > correlation_threshold), f'Got SSIM correlation scores < {correlation_threshold}'
