...
def classic_isp_factory_dict_default():
    return utils.factory({"class": "ClassicISP", "checkpoint": None})


@pytest.fixture(scope="module")
def dnet_factory():
    return utils.factory({"class": "DNet", "checkpoint": "data/models/nip/D90/dnet"})
...
def test_baseline_dcs():
    for baseline in [16, 32, 64]:
        ...
        assert (
            dcn.latent_shape[-1] == baseline
        ), f"DCN: Latent shape does not the expected number of channels ({baseline})!"
...
def test_dcn(raw_data):
    ...
    assert np.all(
        ssims > SSIM_DCN_THRESHOLD
    ), f"DCN: Got SSIM scores < {SSIM_DCN_THRESHOLD}"
