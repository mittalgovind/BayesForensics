...
def bpg_compress(input_image_p, q, tmp_dir=None, chroma_fmt="444"):
    ...
    if tmp_dir:
        ...
        output_image_bpg_p = os.path.join(tmp_dir, input_image_name).replace(
            ".png", "_tmp_bpg.bpg"
        )
    else:
...
def bpg_image_info(p):
    ...
    with open(p, "rb") as f:
        ...
        num_bytes_for_picture = (
            _number_of_bytes_until_eof(f)
            if picture_data_length == 0
            else picture_data_length
        )
