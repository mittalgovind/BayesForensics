                ...
                for qi, q in enumerate(quality_levels):
                    ...
                    if write_files:
                        ...
                        image_path = os.path.join(
                            image_dir, "jpeg_q{:03d}.png".format(q)
                        )
                        imageio.imwrite(
                            image_path, (255 * image_compressed).astype(np.uint8)
                        )
                    ...
                    df = df.append(
                        {
                            ...
                            "quality": q,
                            "ssim": compare_ssim(
                                image, image_compressed, multichannel=True, data_range=1
                            ),
            ...
            for image_id, filename in enumerate(files):
                ...
                for qi, q in enumerate(quality_levels):
                    ...
                    image_compressed = imageio.imread(
                        bpg_helpers.decode_bpg_to_png(bpp_path)
                    ...
                    if write_files:
                        ...
                        image_path = os.path.join(
                            image_dir, "bpg_q{:03d}.png".format(q)
                        )
                        imageio.imwrite(
                            image_path, (255 * image_compressed).astype(np.uint8)
                        )
        ...
        for model_dir in model_dirs:
            ...
            for image_id, filename in enumerate(files):
                ...
                if write_files:
                    ...
                    image_path = os.path.join(
                        image_dir, dcn.model_code.replace("/", "-") + ".png"
                    )
                ...
                df = df.append(
                    {
                        ...
                        "model_dir": os.path.relpath(
                            os.path.split(str(model_dir))[0], model_directory
                        ).replace(dcn.model_filename, ""),
                        ...
                        "bpp": 8
                        * image_bytes
                        / batch_x[image_id].shape[0]
                        / batch_x[image_id].shape[1],
                        "layers": dcn.n_layers if "n_layers" in dcn._h else None,
                        "quantization": "{}-{:.0f}bpf".format(
                            dcn._h.rounding, dcn.latent_bpf
                        ),
...
def plot_curve(
    ...
    for index, dfc in enumerate(df_all):
        ...
        if plot == "fit":
            ...
            for image_no, image_id in enumerate(images):
                ...
                x = dfc.loc[
                    dfc["selected"] & (dfc["image_id"] == image_id), "bpp"
                ].values
                y = dfc.loc[
                    dfc["selected"] & (dfc["image_id"] == image_id), metric
                ].values
                ...
                try:
                    popt, pcov = curve_fit(
                        func, x, y, bounds=fit_bounds, maxfev=10000, sigma=sigma
                    )
                    y_est = func(x, *popt)
            ...
            axes.plot(
                X, yy, styles[index][0], label=labels[index] if add_legend else None
            )
        ...
        if draw_markers:
            ...
            if "entropy_reg" in dfc:
                ...
                detailed_legend = (
                    "full" if marker_legend and index == baseline_count else False
                )
    ...
    title = "{} : {}".format(
        ...
        "{} images".format(n_images)
        if n_images > 1
        else dfc.loc[dfc["selected"], "filename"].unique()[0].replace(".png", ""),
    )
...
def plot_bulk(
    ...
    plot_images = (
        ...
        else [-1] + df_all[0].image_id.unique().tolist()
    )
    ...
    for ax_id, image_id in enumerate(plot_images):
        ...
        for index, dfc in enumerate(df_all):
            ...
            if plot == "fit":
                ...
                for image_no, imid in enumerate(images):
                    ...
                    x = dfc.loc[
                        dfc["selected"] & (dfc["image_id"] == imid), "bpp"
                    ].values
                    y = dfc.loc[
                        dfc["selected"] & (dfc["image_id"] == imid), metric
                    ].values
                    ...
                    try:
                        popt, pcov = curve_fit(
                            func, x, y, bounds=fit_bounds, sigma=sigma, maxfev=100000
                        )
                        y_est = func(x, *popt)
                        mse = np.mean(np.power(y - y_est, 2))
                        mse_l.append(mse)
                        if mse > 0.1:
                            logger.warning(
                                "WARNING Large MSE for {} img=#{} = {:.2f}".format(
                                    labels[index], image_no, mse
                                )
                            )
                ...
                axes.plot(
                    ...
                    yy,
                    styles[index][0],
                    label="{} ({:.3f})".format(labels[index], mse_labels[labels[index]])
                    if add_legend
                    else None,
                )
                y_min = min([y_min, min(yy)]) if update_ylim else y_min
            ...
            if draw_markers > 0:
                ...
                if "entropy_reg" in dfc:
                    ...
                    if image_id >= 0 or draw_markers >= 2:
                        ...
                        detailed_legend = (
                            "full"
                            if marker_legend and index == baseline_count
                            else False
                        )
                        ...
                        if (
                            "entropy_reg" in dfc
                            and len(dfc["entropy_reg"].unique()) > 1
                        ):
                            style_mapping["size"] = "entropy_reg"

                        if (
                            "quantization" in dfc
                            and len(dfc["quantization"].unique()) > 1
                        ):
                            style_mapping["style"] = "quantization"
        ...
        n_images = len(dfc.loc[dfc["selected"], "image_id"].unique())
        if n_images > 1:
            title = "{} for {} images ({})".format(
                plot, n_images, os.path.split(dirname)[-1]
            )
        else:
            title = "\#{} : {}".format(
                image_id,
