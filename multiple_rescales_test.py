...
def predictive_entropy(X):
    ...
    return -np.sum(np.multiply(means, np.divide(np.log(np.clip(means, 1e-12, None)), np.log(n_passes))))
...
def mutual_information(X):
    ...
    exp_value = np.divide(np.sum(np.multiply(probs, np.divide(np.log(np.clip(probs, 1e-12, None)), np.log(n_passes)))), n_passes)
...
np.random.seed(0)
files = np.random.choice([TARGET + '/' + f for f in os.listdir(TARGET) if f[-4:] == '.png'], 250)
...
df = pd.DataFrame(columns=['file', 'true_sf', 'method', 'predicted_sf', 'variation_ratio', 'predictive_entropy', 'mutual_information'])
for sf in classes:
    for method in methods:
        print(f'SF: {sf}, Method: {method}')
        rescaled = np.asarray(tf.image.resize(imgs, [int(sf * patch_size), int(sf * patch_size)], method=method))

        if not loaded:
            preds = model(tf.expand_dims(rescaled[0].astype(float), axis=0), training=False).numpy()
        ...
        for i in range(len(rescaled)):
            ...
            for j in range(num_runs):
                preds.append(softmax(model(tf.expand_dims(rescaled[j].astype(float), axis=0), training=False).numpy()[0]))
            
            df = df.append({'file': files[i],
                            'true_sf': sf,
                            'method': method,
                            'predicted_sf': classes[np.argmax(np.array([np.sum(c) / len(preds) for c in np.array(preds).T]))],
                            'variation_ratio': variation_ratio(preds),
                            'predictive_entropy': predictive_entropy(preds),
                            'mutual_information': mutual_information(preds)}, ignore_index=True)
